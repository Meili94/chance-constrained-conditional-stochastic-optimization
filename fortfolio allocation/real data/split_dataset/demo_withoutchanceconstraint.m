%% Portfolio problems by CSO without chance constraint. 
%% Use the ADMM algorithm to slove.
...
%% ADMM  algorithm
%parameter between mean and variance
lambda = 0.5;  
%regularization parameter
r = 0.1; 
sigma = 100;
%% intial value
maxiter = 2000; tol = 1e-4; tau = 1.618; 
y = zeros(m,1); w = zeros(m,1);
...
Prob = 1-length(find(B*x-alpha<=0))/n;
Prob_test = 1-length(find(A_test*x-alpha<=0))/n_test;
