%% condtional stochastic optimization with LAD loss (use CVX package) and condtional sampling
...
for i = 1:n
    for j = 1:Numeta
eta(j,:)  = mvnrnd(A(i,:),sigma_s*eye(m));
    end
    AE(i,:) = sum(eta,1)/Numeta;
end
A = AE;
%% solving LAD loss by CVX package
cvx_precision(1e-3)
cvx_begin
variable x(m)
...
cvx_end
%% result
bound_upp = 0.5;%the radius of true decision variable
